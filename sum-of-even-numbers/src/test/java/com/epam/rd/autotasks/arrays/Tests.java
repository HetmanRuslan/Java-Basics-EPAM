...
public class Tests {
    ...
    public static Stream<Arguments> testCases() {
        return Stream.of(
                arguments(228, new int[]{1, 3, 2, 8, 15, 199}),
                arguments(227, new int[]{1, 3, 2, 8, 198, 15}),
                arguments(1000, IntStream.generate(() -> 1).limit(1000).toArray()),
                arguments(2000, IntStream.generate(() -> 2).limit(1000).toArray()),
                arguments(0, IntStream.generate(() -> 0).limit(1000).toArray()),
                arguments(500500, IntStream.iterate(1, i -> i + 1).limit(1000).toArray()),
                arguments(9560, IntStream.iterate(1, i -> i + 3).limit(80).toArray()),
                arguments(1048575, IntStream.iterate(1, i -> i + i).limit(20).toArray()),
                arguments(0, IntStream.iterate(1, i -> -i).limit(80).toArray()),
                arguments(-3080, IntStream.iterate(1, i -> i - 1).limit(80).toArray()),
                arguments(50, IntStream.iterate(50, i -> i - 1).limit(100).toArray()),
                arguments(-4900, IntStream.iterate(50, i -> i - 2).limit(100).toArray()),
                arguments(-9850, IntStream.iterate(50, i -> i - 3).limit(100).toArray())
        );
    }
